...
1. **Find the artifact**: Use `gh api` to list artifacts for the given CI run:
   ```bash
   gh api repos/apache/skywalking-graalvm-distro/actions/runs/<RUN_ID>/artifacts --jq '.artifacts[] | {id: .id, name: .name}'
   ```
...
2. **Download and extract**: Download the artifact zip and extract it:
   ...
   gh api repos/apache/skywalking-graalvm-distro/actions/artifacts/<ARTIFACT_ID>/zip > artifact.zip
