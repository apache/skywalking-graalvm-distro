...
Your GPG key must use an `@apache.org` email address. The release script verifies this.
...
```
pre-release.sh ──push──> GitHub Actions CI ──wait──> release.sh ──> [VOTE] email
```

1. **`pre-release.sh`** — Bump version, tag, bump to next SNAPSHOT
2. **GitHub Actions CI** — Automatically builds Linux native binaries and creates GitHub Release
3. **`release.sh`** — Build macOS binary, create source tarball, sign, upload to SVN, generate vote email
