...
/**
 * Pre-compilation test for LAL files containing network-profiling-slow-trace rules.
 * These rules are now in mesh-dp.yaml, envoy-als.yaml, and k8s-service.yaml
 * (upstream removed the standalone network-profiling.yaml).
 */
class LALNetworkProfilingTest extends LALScriptComparisonBase {
    ...
    static void loadRules() throws Exception {
        RULES = loadLALRules("mesh-dp.yaml");
    }
