...
/**
 * Comparison test for otel-rules/banyandb/banyandb-service.yaml.
 *
 * This file exercises:
 * <ul>
 ...
 *   <li>Scope: SERVICE only with service(['host_name'], Layer.BANYANDB)
 *       (no instance scope, unlike banyandb-instance.yaml)</li>
 ...
 * </ul>
 */
class BanyandbServiceTest extends MALScriptComparisonBase {

    private static final String YAML_PATH =
        "otel-rules/banyandb/banyandb-service.yaml";
    ...
    /**
     * Build the full input map covering all sample names in banyandb-service.yaml.
     ...
     *
     * # --- CPU (needs method label for sum(['method','host_name','service_instance_id'])) ---
     * banyandb_system_cpu_num{host_name="banyandb-host",service_instance_id="banyandb-node-1:17912",method="query"} 8.0
     *
     * # --- Error metrics ---
    ...
    private static ImmutableMap<String, SampleFamily> buildInput(
        ...
        return ImmutableMap.<String, SampleFamily>builder()
            ...
            // Memory — tagEqual('kind','total'), sum(['host_name'])
            ...
            // CPU count — needs method label for sum(['method','host_name','service_instance_id'])
