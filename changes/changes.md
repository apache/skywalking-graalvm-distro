...
- Register the distro-only Armeria handlers (`UnsupportedAdminFeatureHandler`, `BundledRuleCatalogHandler`) for native-image reflection in `reachability-metadata.json`, and the LAL effective input types (Envoy `HTTPAccessLogEntry` / `TCPAccessLogEntry`) that the log `DSL` replacement resolves by name. Armeria builds its annotated routes by reflection, and these classes are not on the build-time precompiler's classpath, so without the entries their routes were never registered.
