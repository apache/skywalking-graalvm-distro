 ...
 * Original: skywalking/oap-server/server-library/library-module/src/main/java/.../module/ModuleDefine.java
 * Repackaged into library-module-for-graalvm via maven-shade-plugin (replaces original .class in shaded JAR).
 *
 * Change: Added {@link #prepare(ModuleManager, ModuleProvider, ApplicationConfiguration.ModuleConfiguration,
 * TerminalFriendlyTable)} overload that accepts a pre-selected provider directly, bypassing ServiceLoader
 * discovery. This enables fixed module wiring for GraalVM without reflection hacks.
 * Why: The distro uses FixedModuleManager with directly-constructed modules and providers.
 * The upstream prepare() method requires ServiceLoader to find providers, which we bypass entirely.
 */
