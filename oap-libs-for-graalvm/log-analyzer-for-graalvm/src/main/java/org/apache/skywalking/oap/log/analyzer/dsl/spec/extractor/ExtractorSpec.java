...
public class ExtractorSpec extends AbstractSpec {
    ...
    // ── Closure-based methods (original upstream, used by Groovy test path) ──
    ...
    // ── Consumer-based overloads (transpiled LAL code) ──

    @SuppressWarnings("unused")
    public void metrics(final Consumer<SampleBuilder> body) {
        if (BINDING.get().shouldAbort()) {
            return;
        }
        final SampleBuilder builder = new SampleBuilder();
    ...
    @SuppressWarnings("unused")
    public void slowSql(final Consumer<SlowSqlSpec> body) {
        if (BINDING.get().shouldAbort()) {
            return;
        }
        LogData.Builder log = BINDING.get().log();
    ...
    @SuppressWarnings("unused")
    public void sampledTrace(final Consumer<SampledTraceSpec> body) {
        if (BINDING.get().shouldAbort()) {
            return;
        }
        LogData.Builder log = BINDING.get().log();
    ...
    private void dispatchMetrics(final SampleBuilder builder) {
    ...
    private void finishSlowSql(final DatabaseSlowStatementBuilder builder,
                                final LogData.Builder log) {
