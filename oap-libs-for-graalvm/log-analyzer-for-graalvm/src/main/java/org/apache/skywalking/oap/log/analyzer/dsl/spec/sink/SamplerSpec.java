...
public class SamplerSpec extends AbstractSpec {
    ...
    // ── Closure-based methods (original upstream) ──
    ...
    public void rateLimit(final GString id,
        ...
        final Sampler sampler = rateLimitSamplers.computeIfAbsent(
    ...
    public void possibility(final int percentage,
        ...
        final Sampler sampler = possibilitySamplers.computeIfAbsent(
            percentage, $ -> new PossibilitySampler(percentage).start());
    ...
    // ── Consumer-based overloads (transpiled LAL code uses String, not GString) ──

    @SuppressWarnings("unused")
    public void rateLimit(final String id,
                          final Consumer<RateLimitingSampler> body) {
        if (BINDING.get().shouldAbort()) {
            return;
        }

        final RateLimitingSampler sampler = (RateLimitingSampler) rateLimitSamplersByString
            .computeIfAbsent(id, $ -> new RateLimitingSampler(rlsResetHandler).start());
    ...
    @SuppressWarnings("unused")
    public void possibility(final int percentage,
                            final Consumer<PossibilitySampler> body) {
        if (BINDING.get().shouldAbort()) {
            return;
        }

        final PossibilitySampler sampler = (PossibilitySampler) possibilitySamplers
            .computeIfAbsent(percentage, $ -> new PossibilitySampler(percentage).start());
